...
DATE_TEXT_PATTERNS = [
    ...
    r"Updated\s*:\s*([A-Za-z]{3,9}\s+\d{1,2},\s+\d{4})",
    r"(\d{4}-\d{2}-\d{2})",
]
...
def allowed_title(title: str) -> bool:
    t = (title or "").lower()
    if "product manager" not in t:
        return False
    if "senior" in t or re.search(r"\bsr\.?\b", t):
        ...
        return False
    # Exclusions
    if any(x in t for x in ["product marketing manager", "program manager", "project manager", "product owner", "product analyst"]):
        return False
    return True
...
def same_site_job_link(career_url: str, href: str) -> bool:
    ...
    try:
        ...
        hu = urlparse(href)
        return cu.netloc == hu.netloc and any(s in hu.path.lower() for s in ["/job", "/jobs", "/careers", "/positions", "/opportunities"])
    except Exception:
        return False
...
def append_jobs(csv_path: str, rows: List[Tuple[str, str, str]]):
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Append up to 10 recent (<=30d) Senior PM postings to jobs.csv")
    parser.add_argument("--companies", type=str, required=True)
    parser.add_argument("--out", type=str, default="jobs.csv")
    ...
    if not results:
        log("No recent Senior Product Manager postings found within 30 days.")
        sys.exit(2)

    append_jobs(args.out, results[: args.max])
    log(f"Appended {len(results[: args.max])} rows to {args.out}")
